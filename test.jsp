...
<%
	...
	try { // 例外處理
			...
			try {	// 例外處理
				...
				Connection con=DriverManager.getConnection(url,"root","12345678"); // 連接 MySQL 資料庫（帳號：root；密碼：12345678）
				if(con.isClosed()) // con.isClosed() 回傳 ture 代表連線關閉
				   out.println("警告：連線建立失敗！"); // 連線關閉代表連線沒有被建立
				else { // 連線成功建立就可以開始取得與更新資料庫的資料
					sql="use `mmm`"; // SQL 語法：使用資料庫
					con.createStatement().execute(sql); // 執行上一行的 SQL
					sql="select * from `product`"; // SQL 語法：取得 counter 資料表中的所有資料
